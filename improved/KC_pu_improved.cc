...
KCU_type *KCU_Init(float fPhi, double alpha,unsigned long samples_num, int sketch_bucket_num) {
    ...
    for (i = 0; i < k; i++) {
        ...
        for (j = 0; j < SKETCH_IN_BUCKET; j++) {
            result->items[i].l_x[j] = 0;
            result->items[i].max_cell[j].item = UINT_MAX;
            result->items[i].max_cell[j].epoch = 0;
            result->items[i].max_cell[j].vect_cnt = 0;
            result->items[i].sketch[j] = dds_psketch_init(COLLAPSINGALL_MAPSTORE, alpha, 0.0,INT_MAX);
        }
    }
    return (result);
}
...
std::vector<int> KCU_ComputeMem(KCU_type *kcu) {
    ...
    int udd_memory_usage = kcu->k * SKETCH_IN_BUCKET * (56+56+32);//psketch+mapsotre+mapping
...
#ifdef RS_IN_CELL
    //RS_IN_CELL
    RS_memory += (sizeof(std::vector<sampleEntry>)+ sizeof(unsigned))* kcu->k * SKETCH_IN_BUCKET;//cell attribute
    RS_memory += (int) (pow(0.01, -1)) * kcu->k * SKETCH_IN_BUCKET*sizeof(sampleEntry); //samplesize
...
void KCU_cells_update(KCU_item *item, unsigned int new_item, unsigned int new_epoch, double latency) {
    ...
    // anyway, sample
        ...
        if (current_pos == 0 && item->cells[0].vect_cnt > std::max(3,1 + (int) (theta_ * new_epoch))) {
            // enter max_cell
            int empty_idx = -1;
            for(int i = 0; i < SKETCH_IN_BUCKET; ++i) {
                if (item->max_cell[i].item == UINT_MAX) {
                    empty_idx = i;
                    break;
                }
            }
            ...
            if (empty_idx != -1) {
                item->max_cell[empty_idx] = item->cells[0];
                ...
                item->l_x[empty_idx] = ts;
                dds_sketch_update_mask(item->sketch[empty_idx], latency, 1);
                ...
                int min_idx = -1;
                for(int i = 0; i < SKETCH_IN_BUCKET; ++i) {
                    if (item->max_cell[i].vect_cnt < min_cnt) {
                        min_cnt = item->max_cell[i].vect_cnt;
                        min_idx = i;
                    }
                }

                if (item->cells[0].vect_cnt > min_cnt) {

                    double thresh = KCU_epsilon2 * (new_epoch - item->max_cell[min_idx].epoch) /
                                    (item->max_cell[min_idx].vect_cnt+1);
                    if (g_rand.uniform()< thresh) {

                        item->index_map.erase(item->cells[0].item);
                        item->index_map[item->max_cell[min_idx].item] = 0;

                        std::swap(item->max_cell[min_idx], item->cells[0]);
                        dds_psketch_destroy((dds_psketch *)item->sketch[min_idx]);

                        item->sketch[min_idx] = dds_psketch_init(COLLAPSINGALL_MAPSTORE, item->alpha, 0.0,INT_MAX);


                        dds_sketch_update_mask(item->sketch[min_idx], latency, 1);
...
void KCU_collapse(KCU_type *kcu) {
    ...
    int cnt[kcu->k * SKETCH_IN_BUCKET];
    int num_buckets = 0;

    for (int i = 0; i < kcu->k; i++) {
        for (int j = 0; j < SKETCH_IN_BUCKET; j++) {
            int idx = i * SKETCH_IN_BUCKET + j;
            if (kcu->items[i].sketch[j] == nullptr) {
                cnt[idx] = INT_MAX;
                continue;
            }

            int b = dds_store_get_num_buckets(kcu->items[i].sketch[j]->store);
            num_buckets += b;

            if (b <= kcu->max_buckets / (kcu->k * SKETCH_IN_BUCKET)) {
                cnt[idx] = INT_MAX;
                continue;
            }
            cnt[idx] = -b;
    ...
    while (num_buckets > kcu->max_buckets) {
        ...
        for (int i = 0; i < kcu->k * SKETCH_IN_BUCKET; i++) {
            if (cnt[i] != INT_MAX) {
        ...
        int item_idx = minIndex / SKETCH_IN_BUCKET;
        int sketch_idx = minIndex % SKETCH_IN_BUCKET;

        int before_buckets = dds_store_get_num_buckets(kcu->items[item_idx].sketch[sketch_idx]->store);
        num_buckets -= before_buckets;
        dds_mapstore_collapse((struct dds_mapstore *) kcu->items[item_idx].sketch[sketch_idx]->store);
        int after_buckets = dds_store_get_num_buckets(kcu->items[item_idx].sketch[sketch_idx]->store);
        ++collapse_times;
        num_buckets += after_buckets;
        struct dds_bucket_id_mapping *mapping = dds_sketch_get_id_mapping(
            (struct dds_sketch *) kcu->items[item_idx].sketch[sketch_idx]);
        if (mapping == nullptr) {
            continue;
        }
  ...
  void KCU_collapse_SIMD(KCU_type *kcu) {
      int cnt[kcu->k * SKETCH_IN_BUCKET];
      int num_buckets = 0;
      ...
      while (num_buckets > kcu->max_buckets) {
          ...
          if (n > 0) {
              ...
              if (n >= 8) {
                ...
                // SIMD load to register
                  __m256i min_vec = _mm256_loadu_si256((__m256i*)&cnt[0]);
                  for (i = 8; i <= n - 8; i += 8) {
                      __m256i data_vec = _mm256_loadu_si256((__m256i*)&cnt[i]);
                      min_vec = _mm256_min_epi32(min_vec, data_vec);
                  }
                  // Horizontal to get the minimum value
                  int min_vals[8];
...
void KCU_UpdateLatency(KCU_type *kcu, unsigned int newitem, double latency) {
    ...
    for (int i = 0; i < SKETCH_IN_BUCKET; ++i) {
        if (il->max_cell[i].item == newitem) {
            if (il->max_cell[i].epoch < epoch) {
                il->max_cell[i].vect_cnt++;
                il->max_cell[i].epoch = epoch;
            }
            //whenever insert

            dds_sketch_update_mask(il->sketch[i], latency, 1);

#ifdef  RS_IN_CELL
            KCU_addToSample(&il->max_cell[i],latency);
#endif
...
double KCU_QuantileQuery(KCU_type *kcu,unsigned int newitem, double rank) {
    ...
    for (int i = 0; i < SKETCH_IN_BUCKET; ++i) {
        if (newitem == il->max_cell[i].item) {
            bool is_accurate = true;
            double val = dds_sketch_get_quantile((dds_sketch *) il->sketch[i], rank, &is_accurate);
            return val;
        }
    }

    return -1; // item is not monitored
}
...
dds_psketch *dds_psketch_tmp(KCU_type *kcu, unsigned int newitem) {
    ...
    if (max_cell_idx == -1) // item is not in hashtable
        return nullptr;

    dds_psketch *tmpsketch = dds_psketch_copy(il->sketch[max_cell_idx]);

    for (auto const &value: il->max_cell[max_cell_idx].samples) {
        if (value.epoch < il->l_x[max_cell_idx]/KCU_EPOCH_SIZE +1) {
            sample_buff.push_back(value.latency);
            dds_sketch_update_mask((dds_sketch *) tmpsketch, value.latency,
                              (long) (il->max_cell[max_cell_idx].nb_processed/ il->max_cell[max_cell_idx].samples.size()));
        }
    }
    return tmpsketch;
}
...
std::pair<int, std::map<uint32_t, uint32_t>> KCU_Output2(KCU_type * kcu) {
    ...
    for (int i = 0; i < kcu->k; ++i) {
        for (int j = 0; j < SKETCH_IN_BUCKET; ++j) {
            if (!dds_sketch_is_empty((dds_sketch *) kcu->items[i].sketch[j])) {
                res.insert(std::pair<uint32_t, uint32_t>(kcu->items[i].max_cell[j].item, kcu->items[i].max_cell[j].vect_cnt));
                buckets +=dds_store_get_num_buckets(kcu->items[i].sketch[j]->store);
            }
        }
    }
...
std::map<uint32_t, uint32_t> KCU_Output3(KCU_type * kcu) {
    ...
    for (int i = 0; i < kcu->k; ++i){
        ...
        for (int j = 0; j < SKETCH_IN_BUCKET; ++j) {
            if (!dds_sketch_is_empty((dds_sketch *) kcu->items[i].sketch[j])) {
                res.insert(std::pair<uint32_t, uint32_t>
                    (kcu->items[i].max_cell[j].item, kcu->items[i].max_cell[j].vect_cnt));
            }
        }
    }
    return res;
...
void KCU_Destroy(KCU_type *kcu) {
    for (int i = 0; i < kcu->k; i++) {
        for (int j = 0; j < SKETCH_IN_BUCKET; j++) {
            delete(kcu->items[i].sketch[j]); // should done
        }
    }
    free(kcu->items);
    free(kcu->hashtable);
